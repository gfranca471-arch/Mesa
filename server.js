...
function musicPayload(room) {
    return { ...room.musicState, serverTime: Date.now() };
}
...
io.on('connection', (socket) => {
        ...
        try {
            ...
            if (!room) {
                ...
                room = normalizeRoom({
                    ...
                    currentImage: null,
                    tokens: [], sharedMusic: [], sheets: {}, profiles: {},
            ...
            room.players[socket.id] = {
                ...
                cameraOn: false, micOn: false, screenSharing: false, joinedAt: Date.now()
            };
    ...
    socket.on('change-image', async (data) => {
        ...
        const room = rooms[code];
        room.currentImage = String(data.url || '').slice(0, 12 * 1024 * 1024) || null;
