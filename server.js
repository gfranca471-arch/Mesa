...
function normalizeRoom(room, code) {
    if (!room) return null;
    room.code = code;
    room.system = ['vampiro', 'vampirov6', 'lobisomem', 'dnd'].includes(room.system) ? room.system : 'lobisomem';
...
app.get('/health', (req, res) => {
    res.status(200).json({
        ...
        v6AuthorizationConfigured: Boolean(process.env.V6_CAMPAIGN_AUTH_KEY)
    });
});
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', async (data) => {
        try {
            ...
            if (!suppliedPassword) return socket.emit('room-error', 'A senha da sala é obrigatória.');

            let room = await loadRoom(code);
            if (!room) {
                if (role !== 'narrador') return socket.emit('room-error', 'Sala não existe. Peça ao Narrador para criar.');
                const campaignName = cleanLabel(data.campaignName, 100);
                const roomName = cleanLabel(data.roomName, 100);
                if (!campaignName) return socket.emit('room-error', 'Informe o nome da campanha.');
                if (!roomName) return socket.emit('room-error', 'Informe o nome da sala.');
                const campaignKey = keyForLabel(campaignName);
                const roomNameKey = keyForLabel(roomName);
                if (!campaignKey || !roomNameKey) return socket.emit('room-error', 'Nome da campanha ou da sala inválido.');
                const conflict = await roomNameConflict(campaignKey, roomNameKey, code);
                if (conflict) return socket.emit('room-error', `Já existe uma sala chamada “${roomName}” nesta campanha.`);

                const requestedSystem = ['vampiro', 'vampirov6', 'lobisomem', 'dnd'].includes(data.system) ? data.system : 'lobisomem';
                if (requestedSystem === 'vampirov6') {
                    const ownerKey = process.env.V6_CAMPAIGN_AUTH_KEY || '';
                    if (!ownerKey) return socket.emit('room-error', 'Criação de campanhas Vampiro V6 ainda não foi autorizada no servidor.');
                    if (!secretMatches(data.v6Authorization, ownerKey)) return socket.emit('room-error', 'Autorização para criar campanha Vampiro V6 inválida.');
                }

                const passwordFields = passwordRecord(suppliedPassword);
                room = normalizeRoom({
                    ...passwordFields,
                    campaignName, campaignKey, roomName, roomNameKey,
