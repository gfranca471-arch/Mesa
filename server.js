...
async function sendV6ApprovalEmail(req, socket) {
    ...
    if (!user || !pass) throw new Error('SMTP_USER/SMTP_PASS não configurados');
    const host = process.env.SMTP_HOST || 'smtp.gmail.com';
    const port = Number(process.env.SMTP_PORT || 465);
    const secure = String(process.env.SMTP_SECURE || (port === 465 ? 'true' : 'false')).toLowerCase() !== 'false';
    const transport = nodemailer.createTransport({ host, port, secure, auth:{ user, pass } });
    const base = baseUrlFromSocket(socket);
    if (!base) throw new Error('PUBLIC_BASE_URL não configurada e host indisponível');
    const approveUrl = `${base}/v6/approve?id=${encodeURIComponent(req.id)}&token=${encodeURIComponent(req.token)}`;
    const rejectUrl = `${base}/v6/reject?id=${encodeURIComponent(req.id)}&token=${encodeURIComponent(req.token)}`;
...
async function createOrReuseV6Approval({ campaignName, campaignKey, requestedBy, socket }) {
    ...
    req = {
        ...
        campaignName, campaignKey, requestedBy,
    ...
    try { await sendV6ApprovalEmail(req, socket); req.emailed = true; }
...
app.get('/api/v6-approval-status', (req,res) => {
    ...
    res.json({status:item.status, emailed:item.emailed, emailError:item.emailError||'', expiresAt:item.expiresAt});
});
            ...
            if (!room) {
                ...
                if (requestedSystem === 'vampirov6') {
                    ...
                    if (!validApproval) {
                        ...
                        if (!approval.emailed) {
                            return socket.emit('room-error', `Não foi possível enviar o pedido de autorização V6 para ${V6_APPROVAL_EMAIL}. Configure SMTP_USER e SMTP_PASS no Render. ${approval.emailError ? '('+approval.emailError+')' : ''}`);
                        }
                        socket.emit('v6-authorization-pending', { approvalId:approval.id, email:V6_APPROVAL_EMAIL, expiresAt:approval.expiresAt });
                        return;
                    }
